...
describe("Job status & stream routes", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe("GET /api/v1/assignments/client-sql-code-run/status/:taskId", () => {
    ...
    it("returns 400 for non-numeric taskId when authenticated", async () => {
      ...
      const res = await request(app).get(
        "/api/v1/assignments/client-sql-code-run/status/invalid-id",
      );
    ...
    it("returns 403 when user is not owner nor admin", async () => {
      mockGetSession.mockResolvedValue({
        user: { id: "user-1", role: "user" },
        session: { id: "sess-1" },
      });
    ...
    it("returns 200 with job status when user is owner", async () => {
      mockGetSession.mockResolvedValue({
        user: { id: "user-1", role: "user" },
        session: { id: "sess-1" },
      });
  ...
  describe("GET /api/v1/assignments/client-sql-code-run/status/:taskId/stream", () => {
    it("returns 401 without session", async () => {
      mockGetSession.mockResolvedValue(null);
